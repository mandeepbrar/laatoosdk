...
func FlattenMap(mapToFlatten map[string]interface{}, retMap map[string]interface{}, initStr string, separator string) {
	for k, v := range mapToFlatten {
		valMap, ok := v.(map[string]interface{})
		if ok {
			if initStr != "" {
				FlattenMap(valMap, retMap, separator, fmt.Sprintf("%s%s%s", initStr, separator, k))
			} else {
				FlattenMap(valMap, retMap, separator, k)
			}
		} else {
			if initStr != "" {
				k = fmt.Sprintf("%s%s%s", initStr, separator, k)
			}
			retMap[k] = v
		}
	}
