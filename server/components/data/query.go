...
type ScopeRequirement struct {
	...
	//
	// A hop whose Connection differs from the querying component's is HOP-EXECUTOR-ONLY and always
	// will be: a dataconnection is a store boundary, and no provider can compile a join across two
	// stores however capable it is.
	//
	// NOT to be confused with Extension.Namespace above, which is a VOCABULARY namespace naming
	// which provider owns an extension's grammar and has nothing to do with storage.
	//
