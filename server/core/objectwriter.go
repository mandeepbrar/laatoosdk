...
type SerializableWriter interface {
	Bytes() []byte
	Write(ctx ctx.Context, cdc Codec, prop string) (SerializableWriter, error)
	WriteBytes(ctx ctx.Context, cdc Codec, val *[]byte) error
	WriteByte(ctx ctx.Context, cdc Codec, prop string, val *byte) error
	...
	WriteObject(ctx ctx.Context, cdc Codec, prop string) (SerializableWriter, error)
