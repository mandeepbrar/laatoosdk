...
type logPrinter func(ctx core.Context, strlevel string, wh SimpleWriteHandler, level int, msg string, args ...interface{})
...
type SimpleWriteHandler interface {
	Print(ctx core.Context, msg string, level int)
	PrintBytes(ctx core.Context, msg []byte) (int, error)
}

func NewSimpleLogger(wh SimpleWriteHandler) LoggerInterface {
	logger := &SimpleLogger{format: "json", level: INFO, wh: wh}
...
func (log *SimpleLogger) Write(p []byte) (int, error) {
	return log.wh.PrintBytes(nil, p)
}
